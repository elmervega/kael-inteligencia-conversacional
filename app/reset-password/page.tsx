...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="border border-zinc-800 rounded-2xl p-8 bg-zinc-900/20 backdrop-blur-sm text-center space-y-4">
          ...
          {error && (
            ...
            >
              ...
              {error.includes('expiró') && (
                <Link href="/forgot-password" className="text-red-300 underline whitespace-nowrap text-xs">
                  Nuevo enlace →
                </Link>
              )}
            </motion.div>
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white flex items-center justify-center px-4">
      ...
      >
        ...
        <Suspense fallback={
          <div className="border border-zinc-800 rounded-2xl p-8 bg-zinc-900/20 backdrop-blur-sm animate-pulse h-56" />
        }>
          <ResetPasswordForm />
        </Suspense>
      </motion.div>
